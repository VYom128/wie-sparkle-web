...
const HorizontalScrollMembers = ({
            ...
            { x: startX },
            {
              ...
              modifiers: {
                x: gsap.utils.unitize(
                  gsap.utils.wrap(-totalOriginalWidth, 0),
                  "px"
                ),
              },
              onUpdate: () => {
  ...
  return (
    <div className="space-y-20">
      {/* Team Leads */}
      <div>
        <h3 className="text-2xl font-medium text-center mb-12 tracking-tight">
          Team Leads
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-2 sm:gap-8">
          {teamLeads.map((lead, index) => (
            <div
              key={lead.id}
              className="group cursor-pointer"
              onClick={() =>
                setSelectedMember(selectedMember === lead.id ? null : lead.id)
              }
            >
              <div className="glass-card rounded-2xl overflow-hidden transition-all duration-500 group-hover:scale-105 group-hover:shadow-xl group-hover:shadow-primary/20">
                <div className="relative">
                  <img
                    src={lead.image}
                    alt={lead.name}
                    className="w-full h-48 sm:h-64 object-cover transition-transform duration-500 group-hover:scale-110" // Adjusted height for mobile
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-background/80 via-transparent to-transparent" />
                  <div className="absolute bottom-4 left-4 right-4">
                    <h4 className="text-xl font-medium text-foreground mb-1 tracking-tight">
                      {lead.name}
                    </h4>
        ...
        >
          ...
          >
            {duplicatedMembersArray.map((member, index) => (
              ...
              >
                <div
                  className={`glass-card rounded-xl overflow-hidden transition-all duration-700 group-hover:scale-105 group-hover:shadow-xl group-hover:shadow-primary/30 ${
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute bottom-2 left-2 right-2">
                      <h5 className="text-sm font-medium text-foreground mb-1 tracking-tight">
                        {member.name}
                      </h5>
